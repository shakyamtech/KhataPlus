...
export const PartiesPage = ({ type }: { type: "customer" | "supplier" }) => {
      ...
      if (type === "customer") {
        ...
        sortedPurchases.forEach((pu: any) => {
          ...
          items.push({
            ...
            discount: Number(pu.discount || 0),
            discount_percent: pu.discount_percent ?? null,
        ...
        ledgerData.forEach((l: any) => {
          ...
          if (l.entry_type === "purchase") return;
  ...
  if (selected) {
    return (
      <div className="p-3 sm:p-4 md:p-8 max-w-4xl mx-auto space-y-4 pb-20">
        <PageHeader
  ...
  return (
    <div className="p-3 sm:p-4 md:p-8 max-w-5xl mx-auto space-y-4 pb-20">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {filtered.map((p) => (
          <Card key={p.id} className="p-3.5 sm:p-4 shadow-card border border-transparent cursor-pointer hover:shadow-elegant hover:-translate-y-1 hover:border-primary/40 transition-all duration-300 outline-none group overflow-hidden" onClick={() => openLedger(p)}>
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                <PartyAvatarRing name={p.name} balance={p.balance} size="sm" />
                <div className="min-w-0 flex-1">
                  <div className="font-display text-base sm:text-lg truncate leading-tight" title={p.name}>{p.name}</div>
                  {p.phone && <div className="text-xs text-muted-foreground truncate mt-0.5">{p.phone}</div>}
                </div>
              </div>
              <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
                <div
                  className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-full bg-secondary/80 border border-border/50 text-[11px] sm:text-xs font-semibold text-muted-foreground select-none transition-colors hover:text-foreground hover:bg-secondary"
                  title={type === "customer" ? `${p.txCount || 0} वटा बिक्री / अर्डर (Total Orders)` : `${p.txCount || 0} वटा खरिद बिल (Total Purchase Bills)`}
                >
                  {type === "customer" ? (
                    <ShoppingCart className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary" />
                  ) : (
                    <Receipt className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-primary" />
                  )}
                  <span>{p.txCount || 0}</span>
                </div>
                <Button size="icon" variant="ghost" className="h-8 w-8 active:scale-95" onClick={(e) => {
                  e.stopPropagation();
                  openLedger(p).then(() => {
                    openPaymentModal();
                  });
                ...
                <Button size="icon" variant="ghost" className="h-8 w-8 active:scale-95 text-destructive hover:text-destructive" onClick={(e) => { e.stopPropagation(); remove(p.id); }}>
            ...
            <div className="mt-3 flex items-center justify-between bg-secondary/80 rounded-lg px-3 py-2">
              ...
              <span className={`font-bold text-xs sm:text-sm truncate ml-2 text-right ${Number(p.balance) > 0 ? "text-orange-600 dark:text-orange-400" : Number(p.balance) < 0 ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}>
